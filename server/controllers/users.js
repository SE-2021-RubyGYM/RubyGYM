...
module.exports = {
    ...
    createUser: async(req,res)=> {
        ...
        if (!username || !password || !name || !phone){
            return res
                .status(400)
                .json({success:false,message:'Missing information(s)',result: null})
        }
        ...
        try{
            ...
            if(user){
                return res
                .status(400)
                .json({success:false,message:'Username is already exist',result: null})  
            }
        ...
        const newUser = new User({username, password: hashedPassword,name,gender, birthDay,paymentDay,referralCode,phone})
    ...
    login:async (req, res) => {
        ...
        try {
            ...
            if (!user)
                return res
                    .status(400)
                    .json({ success: false, message: 'Incorrect username',result: null })
    ...
    getUserById: async(req,res) => {
        ...
        try{
            ...
            return res
            .json({ success: true, message: 'API OK',result: user  })
    ...
    changePassword :  async(req,res) => {
        ...
        try{
            ...
            const passwordValid = await argon2.verify(oldUser.password,req.body.oldPassword)
            if(!passwordValid){
                return res
                .status(700)
                .json({ success: false, message: 'Old password doesn\'t match',result: null  })
            }
            
            // All good
            newPassword = await argon2.hash(req.body.newPassword);
            const user = await User.findByIdAndUpdate(id, {password: newPassword, updateAt: updateAt}, {new:true});
            if(!user){
                return res
                .status(404)
                .json({ success: false, message: 'User not found',result: null  })
    ...
    changeUserAssessment : async(req,res) => {
        ...
        const id = req.params.id;
        try{
            const user = await User.findByIdAndUpdate(id, {assessment: req.body.assessment, updateAt: updateAt}, {new:true});
            if(!user){
                return res
                .status(404)
                .json({ success: false, message: 'User not found',result: null  })
